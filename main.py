...
6. Documents – важные документы, счета, акты и тому подобное, которые в дальнейшем нужно будет сохранить в специальное хранилище.
...
"""
    ...
    def classification(self, data, use_ai=False):
        ...
        spam_patterns = [
            (r'выигр|розыгрыш|приз|победител|поздрав|подтвердите личность|подозрительн', 5),
            (r'подтвердит[ье]\s+личност|верификация аккаунта|войдите по ссылке|будет\s+заблокирован', 5),
            (r'http://', 2),
            (r'скидк|акция|эксклюзив|немедленно', 5),
