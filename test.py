...
class TestSpamCategory: # сделан
    ...
    @pytest.mark.parametrize("subject,body", [
        ...
        ("Скидка 90% только сегодня", "Подтвердите личность: эксклюзивное предложение, действуйте немедленно"),
        ("Эксклюзивное предложение", "Войдите по ссылке"),
        ("Черная пятница", "Товары со скидкой до 70%, http://fake-link.com/confirm"),
        ("Важное сообщение", "войдите по ссылке http://scam-site.ru"),
    ])
...
class TestSupportCategory: # сделан
    ...
    @pytest.mark.parametrize("subject,body", [
        ...
        ("Принтер не печатает", "Выдает ошибку 5000"),
        ...
        ("Помогите!", "Что делать, если сломался ноутбук?"),
        ("VPN не работает", "Помогите настроить VPN, после обновления перестал подключаться"),
    ])
...
class TestInfoCategory:
    ...
    @pytest.mark.parametrize("subject,body", [
        ...
        ("Напоминание", "Плановые работы: система будет недоступна в выходные с 2:00 до 6:00"),
...
class TestDocumentsCategory: # сделан
    ...
    @pytest.mark.parametrize("subject,body", [
        ("Счёт на оплату №123", "Оплатите счёт до 30 июня, инвойс во вложении"),
        ("Инвойс от поставщика", "Invoice №INV-2024-001"),
        ("Счет №45 от 01.06", "Бухгалтер: оплата на сумму 50 000 рублей"),
        ("Договор на подпись", "Просим подписать договор оказания услуг"),
        ("Контракт с заказчиком", "Документ во вложении: согласование финальной версии"),
       ("Техническое задание", "Документ: ТЗ на разработку системы"),
        ("Акт выполненных работ", "Документ: просим подписать акт"),
        ("Спецификация", "Описание требований к оборудованию, документ во вложении"),
        ("Документы во вложении", "Прикрепляем скан договора"),
        ("Проверьте файл", "Во вложении документ с подробностями"),
        ("Реквизиты для оплаты", "Документ: банковские реквизиты организации"),
...
class TestCategoryCompetition:
    ...
    @pytest.mark.parametrize("subject,body,expected", [
        ("Не открывается счёт", "Прикрепляю счет на оплату, но файл не открывается, программа зависает", "Documents"),
        ("Нет доступа к принтеру", "Не могу печатать, выдает ошибку доступа", "Access"),
        ("Выиграйте принтер!", "Перейдите по ссылке и получите принтер в подарок", "Spam"),
        ("Критическая ошибка", "Система не работает, срочно чините", "Incidents"),
        ("Отчет с вложением", "Документ: еженедельный отчет во вложении, прикрепляю файл", "Documents"),
    ])
