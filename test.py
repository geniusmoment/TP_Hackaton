...
class TestIncidentsCategory: # сделан
    ...
    @pytest.mark.parametrize("subject,body", [
        ...
        ("Система не отвечает", "Сервер не запускается уже 10 минут"),
        ...
        ("Alert!","Корпус затопило, немедленно покиньте здание"),
...
class TestSpamCategory: # сделан
    ...
    @pytest.mark.parametrize("subject,body", [
        ...
        ("Верификация аккаунта", "Войдите по ссылке для подтверждения"),
...
class TestSupportCategory: # сделан
    ...
    @pytest.mark.parametrize("subject,body", [
        ...
        ("Outlook не работает", "Не отправляются письма"),
        ("Excel вылетает", "При открытии большой таблицы"),
        ("Не открывается файл", "Adobe Reader пишет ошибку"),
